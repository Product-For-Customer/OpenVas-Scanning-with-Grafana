...
-- =========================================================
-- 6) ลบทริกเกอร์/ฟังก์ชันเก่าแบบปลอดภัย
-- =========================================================
DO $$
BEGIN
    IF to_regclass('public.tasks') IS NOT NULL THEN
    ...
    ELSE
        RAISE NOTICE 'public.tasks not found, skip dropping triggers';
    END IF;
END
$$;
...
BEGIN
    IF to_regclass('public.tasks') IS NOT NULL THEN
        ...
        RAISE NOTICE 'trigger_scan_status_notify created successfully';
    ELSE
