...
const Service: React.FC = () => {
  ...
  const [originalData, setOriginalData] = useState<SendEmailResponse | null>(
    null
  );
  ...
  const fetchSendEmail = async (mode: "initial" | "refresh" = "initial") => {
    ...
    try {
      ...
      if (mode === "refresh" && isMountedRef.current) {
        setRefreshing(true);
      }
                    ...
                    <div className="space-y-2.5">
                      <div className="rounded-2xl border border-gray-200 bg-[#f8fbff] p-3.5 dark:border-white/10 dark:bg-[#0b1425]/80">
                        ...
                        <p className="text-[12px] text-gray-500 dark:text-white/45">
                          This page is designed for one email sender
                          configuration only.
                        </p>
                      </div>
                      ...
                      <div className="rounded-2xl border border-gray-200 bg-[#f8fbff] p-3.5 dark:border-white/10 dark:bg-[#0b1425]/80">
                        <div className="mb-2 flex items-center gap-2">
                          <FiUser className="text-[13px] text-sky-600 dark:text-sky-300" />
                          <span className="text-[10px] font-semibold uppercase tracking-[0.14em] text-gray-400">
                            App User ID
                          </span>
                        </div>
                        <p className="break-all text-[13px] font-medium text-[#2b2f45] dark:text-white/85">
                          {loading
                            ? "Loading..."
                            : originalData?.app_user_id
                              ? originalData.app_user_id
                              : "-"}
                        </p>
                        <p className="mt-1.5 text-[11px] text-gray-500 dark:text-white/40">
                          This value is assigned automatically from the logged-in
                          user.
                        </p>
                      </div>
                  ...
                  <div className="rounded-3xl border border-gray-200/80 bg-white/80 p-3.5 shadow-sm dark:border-white/10 dark:bg-white/6">
                    ...
                    <div className="space-y-2.5 text-[12px] leading-5.5 text-[#4f5366] dark:text-white/60">
                      ...
                      <div className="rounded-2xl border border-dashed border-violet-200 bg-violet-50/70 p-3.5 dark:border-violet-400/15 dark:bg-violet-500/10">
                        Keep this configuration updated only when the email
                        sender account changes.
                      </div>

                      <div className="rounded-2xl border border-dashed border-sky-200 bg-sky-50/70 p-3.5 dark:border-sky-400/15 dark:bg-sky-500/10">
                        App User ID is not editable from this page. It will be
                        updated automatically by the authenticated backend
                        session.
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="grid h-full grid-cols-1 gap-4">
                          ...
                          <div className="mt-2.5">
                            {formData.email.trim().length === 0 ? (
                              <p className="text-[11px] text-gray-500 dark:text-white/40">
                                Please enter an email address for outgoing
                                system messages.
                              </p>
                            ) : isValidEmail ? (
                        ...
                        <div className="rounded-3xl border border-gray-200 bg-[#f8fbff] p-3.5 dark:border-white/10 dark:bg-[#0b1425]/80 sm:p-4.5">
                          ...
                          <p className="mt-2.5 text-[11px] text-gray-500 dark:text-white/40">
                            It is recommended to use an app password for better
                            email security.
                          </p>
                        </div>
                        ...
                        <div className="rounded-3xl border border-gray-200 bg-white p-3.5 dark:border-white/10 dark:bg-white/5 sm:p-4.5">
                          ...
                          <div className="mt-3.5 rounded-2xl border border-cyan-200 bg-cyan-50/70 px-3.5 py-3 dark:border-cyan-400/15 dark:bg-cyan-500/10">
                            <div className="flex items-start gap-2.5">
                              <FiUser className="mt-0.5 shrink-0 text-[14px] text-cyan-700 dark:text-cyan-300" />
                              <p className="text-[12px] leading-5 text-cyan-800 dark:text-cyan-200/90">
                                AppUserID is not sent from this form. The backend
                                will use the currently logged-in user when this
