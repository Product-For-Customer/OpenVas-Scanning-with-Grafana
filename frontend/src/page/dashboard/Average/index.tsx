...
const CustomTooltip = ({
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white px-3 py-2 shadow-lg">
      <p className="text-sm font-semibold text-[#1f2240] mb-1">{label}</p>
      <div className="space-y-1 text-xs">
        <p className="text-[#6f5be8]">
          Regular paid course: <span className="font-semibold">{regular.toLocaleString()}</span>
        </p>
        <p className="text-[#7ac65d]">
          On sale course: <span className="font-semibold">{sale.toLocaleString()}</span>
        </p>
      </div>
    </div>
  );
...
const AverageEnrollment: React.FC = () => {
  const [range, setRange] = useState<"This Month" | "Last Month" | "This Quarter">("This Month");
    ...
    <section className="rounded-[22px] bg-[#f7f7f8] border border-gray-200/80 shadow-sm p-5 sm:p-6">
      ...
      <div className="flex items-center justify-between gap-3 mb-5">
        ...
        <div className="relative">
          <select
            value={range}
            onChange={(e) => setRange(e.target.value as "This Month" | "Last Month" | "This Quarter")}
            className="
              appearance-none
              h-11 rounded-xl border border-gray-200 bg-[#fbfbfc]
              px-4 pr-10 text-[14px] sm:text-[15px]
          ...
          <MdKeyboardArrowDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xl text-gray-400" />
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-8 gap-y-2 mb-4 pl-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px] text-gray-700">Regular paid course</span>
        </div>

        <div className="flex items-center gap-2">
          <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#7ac65d]" />
          <span className="text-[14px] text-gray-700">On sale course</span>
        </div>
      </div>
      ...
      <div className="h-70 sm:h-85">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <CartesianGrid stroke="#e9e9ee" strokeDasharray="0" vertical={false} />
            <XAxis
              dataKey="week"
            ...
            <YAxis
              ...
              width={46}
            />
            <Tooltip content={<CustomTooltip />} cursor={{ stroke: "#d7d7e2", strokeDasharray: "4 4" }} />
